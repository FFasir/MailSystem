...
class ReplyTemplateManager(context: Context) {
    ...
    private fun getDefaultTemplates(): List<ReplyTemplate> {
        return listOf(
            ReplyTemplate(UUID.randomUUID().toString(), "确认收到", "您好，邮件已收到，我会尽快处理。", "通用"),
            ReplyTemplate(UUID.randomUUID().toString(), "稍后回复", "您好，我现在比较忙，稍后会详细回复您。", "通用"),
            ReplyTemplate(UUID.randomUUID().toString(), "谢谢", "谢谢您的分享！", "感谢"),
            ReplyTemplate(UUID.randomUUID().toString(), "同意", "我同意您的提议。", "商务")
        )
    }
}
